...
if [ ! -d data ]; then
    ...
    #xml2json.py --path run --depth 1 --encoding cp437 XML_p.xml
fi
...
do
    ...
    if [ x${COUNT} = "x0" ]; then
        ...
        < ${FILE} parallel -j 1 --files --pipe -l 65536 cat | parallel "post-simple.py {} --core ${ID} --seq {#} --rename-id; rm {}; sleep 1"
    fi
done
