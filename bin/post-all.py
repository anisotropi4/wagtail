...
if DEBUG:
    ...
    RENAMEID = False
    #CORE = 'nested'
    CWD = '/home/diablo/wagtail/TPS'

def wtt_datetime2(this_column):
    """wtt_datetime: convert datatime to ISO8601 UTC/Z string"""
    return pd.to_datetime(this_column, box=True).strftime('%Y-%m-%dT%H:%M:%SZ')
...
def get_type(this_series):
    ...
    if s.apply(is_list_like).any():
        s = pd.Series(pd.core.common.flatten(s))
    try:
    ...
    for c, t in schema_v('type', this_schema).items():
        ...
        if t == 'pdate':
            try:
                ...
                idx_1 = df1[c].apply(is_list_like)
                df1.loc[idx_1, c] = df1.loc[idx_1, c]\
                                       .apply(wtt_datetime2).apply(list)
                idx_2 = df1[c].notna() & ~idx_1
                df1.loc[idx_2, c] = df1.loc[idx_1, c].apply(wtt_datetime2)
